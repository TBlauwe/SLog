#pragma once
// Credits : Stack overflow - HolyBlackCat answer :
...
#endif
    template <typename T> [[nodiscard]] constexpr std::string_view RawTypeName()
    {
#ifndef _MSC_VER
...
#pragma clang diagnostic pop
    ...
    static constexpr auto type_name_storage = [] {
        ...
        std::copy_n(RawTypeName<T>().data() + type_name_format.junk_leading, ret.size() - 1,
                    ret.data());
        return ret;
    }();
} // namespace impl
...
template <typename T> [[nodiscard]] constexpr const char *type_name_cstr()
{
    return impl::type_name_storage<T>.data();
}

template <typename T> [[nodiscard]] std::string friendly_type_name()
{
